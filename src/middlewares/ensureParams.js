...
import { ValidationError } from "../errors/CommonError.js";
    ...
    const middleware = {
        ...
        build() {
            ...
            return (req, res, next) => {
                ...
                if (missingQuery) {
                    return next(new ValidationError(`필수 쿼리가 누락되었습니다. (${missingQuery})`));
                }

                //body 필드 검사
                const missingField = _checkObj(req.body, required.body);
                if (missingField) {
                    return next(new ValidationError(`필수 필드가 누락되었습니다. (${missingField})`));
                }

                //param 필드 검사
                const missingParam = _checkObj(req.params, required.params);
                if (missingParam) {
                    return next(new ValidationError(`필수 파라미터가 누락되었습니다. (${missingParam})`));
                }
                
                //숫자 자료형 검사
                for (const element of required.shouldNumberList) {
                    if (element in req.query && !_isNumber(req.query[element])) {
                        return next(new ValidationError(`쿼리 형식이 잘못되었습니다. (${element})`));
                    }
                    if (element in req.body && !_isNumber(req.body[element])) {
                        return next(new ValidationError(`필드드 형식이 잘못되었습니다. (${element})`));
                    }
                    if (element in req.params && !_isNumber(req.params[element])) {
                        return next(new ValidationError(`파라미터 형식이 잘못되었습니다. (${element})`));
                    }
                };
