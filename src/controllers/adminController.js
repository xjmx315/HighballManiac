...
const updateTable = async (req, res) => {
    ...
    try {
        ...
        if (info.length === 0){
            return res.status(200).json(new CommonResponse().setMessage('모든 행을 정상적으로 처리했습니다. '));
        }
        else{
            return res.status(200).json(new CommonResponse().setMessage('일부 행 처리에 실패했습니다. ').setData({info}));
        }
